...
const useStyles = makeStyles((theme) => ({
  ...
  contentContainer: {
    width: '100%',
    maxWidth: '1200px', // Limit the width of your layout
...
const Dashboard = () => {
  ...
  //       console.log('Formated File Content:', formattedContent);
  //       const stripedContent = formattedContent.replace('SMILES,','').replace('smiles,','')
  //       setFileContent(stripedContent); // Save formatted content to state
  //       console.log('Stripped File Content:', stripedContent);
  //     };
      ...
      reader.onload = (event) => {
        ...
        if (file.name.endsWith('.csv')) {
          ...
          Papa.parse(fileContent, {
            complete: (results) => {
              // `results.data` is an array of rows; each row is an array of columns
              console.log("results.data",results.data);
              const csvdatarows = results.data;
              let smilesIndex = -1;
              for(let i=0;i<csvdatarows[0].length;i++){
                if(csvdatarows[0][i].toLowerCase().trim()=="smiles"){
                  smilesIndex = i;
                }
              }
              ...
              if(smilesIndex!==-1){
                for(let i=1;i<csvdatarows.length;i++){
                  processedSmiles.push(csvdatarows[i][smilesIndex])
                }
              }
          ...
          setFileContent(processedSmiles.join(', ')); // Save processed SMILES content to state
          console.log('Processed SMILES Content:', processedSmiles);
        } else {
  ...
  >
    ...
    <Grid item xs={12} style={{ width: '100%', margin: '0 auto' }}>
      ...
      This AI-assisted QSAR model interface provides a powerful tool for simultaneously predicting multi-organ toxicities in humans. Six endpoints have been selected to represent various human organ-level adverse outcomes, including cardiotoxicity, developmental toxicity, hepatotoxicity, nephrotoxicity, neurotoxicity, and reproductive toxicity. These predictions are essential for assessing multiple in vivo organ toxicities, improving success rates in the early stages of drug development, and consequently mitigating drug failure risks.

      To get started, you can choose whether to include Tox21 in vitro high-throughput screening bioactivity assay data. If you decide to include Tox21 data, simply select the name, CAS, or SMILES of chemicals to receive toxic or non-toxic prediction results for human health outcomes from our AI-QSAR model. If you prefer not to include Tox21 data, you can upload your own CSV file containing the SMILES of chemicals or simply select the name, CAS, or SMILES of chemicals to receive prediction results.
      </Typography>
    </Grid>
      ...
      <Grid item xs={12}>
        ...
        >
          ...
          <Typography variant="body2" sx={{ mt: 1 }}>
            Please upload a CSV file containing a column labeled &apos;<strong>SMILES</strong>&apos; or &apos;<strong>smiles</strong>&apos;
          </Typography>
          <Typography variant="body2" sx={{ mt: 1 }}>
  ...
  <Grid item xs={12} style={{ marginTop: '20px' }}>
    {response.map((res, resIndex) => (
      <Alert key={resIndex} severity="info" style={{ marginBottom: '20px' }}>
        ...
        >
          {fileContent==="" ? "CAS: "+options['CAS'][data] : "SMILES: " + fileContentLst[resIndex]}
